...
| Boundary | Evidence |
|---|---|
| Provider isolation | Public runtime exports expose only the shared `ModelProvider` contract and configured factory; OpenAI and AI SDK imports remain in the adapter module |
| Configuration | Only the `openai` provider, a bounded model name, and a printable secret key are accepted; incomplete input, unknown bindings, unsupported providers, and base-URL overrides fail closed |
...
| Provider behavior | The real adapter uses OpenAI through the AI SDK, disables response storage, omits reasoning summaries, requests strict structured output, and has no configurable endpoint override |
...
against the pinned AI SDK/OpenAI packages and is created by the same environment-selected factory.
